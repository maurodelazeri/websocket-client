...
public:
    ...
    public:
        ...
        [[nodiscard]] bool valid() const noexcept { return !_discard; }
    ...
    private:
        ...
        slot(ring_buffer<T> *buf, size_t start, size_t size)
                : _start(start), size_(size), _buf(buf) {}
    ...
    ring_buffer(size_t size)
            : buffer_(new T[size]), size_(size), mask_(size - 1) {
    ...
    [[nodiscard]] bool empty() const noexcept { return cursor_ == 0; }
    ...
    [[nodiscard]] size_t available() const noexcept { return cursor_.load(std::memory_order_relaxed); }
...
public:
    ring_string_buffer(size_t size)
            : buffer_(new char[size]), size_(size), mask_(size - 1), writing_{false} {
...
public:
    object_pool(size_t size)
            : data_(new T[size]), end_(data_ + size), index_(size), size_(size), mask_(size - 1) {
